...
public class GrassAttack extends PokeAttack {
    ...
    private class PrivateAttackInstance extends AttackInstance {
        ...
        public PrivateAttackInstance(PokeAttack pokeAttack, PokemonEntity pokemonEntity, LivingEntity target, CompletableFuture<Boolean> future, boolean isSpecial, int attackSpeed, float power) {
            ...
            particleMain = loadParticle(target.level(), "block{block_state:{Name:moss_block}}");
            particleAlt = ParticleTypes.CHERRY_LEAVES;
        }
        ...
        protected void tick() {
            ...
            AABB aabb = target.getBoundingBox().inflate(0.5, 0.0, 0.5).contract(0, target.getBbHeight()-0.05, 0);
            for(int i = 0; i < 10 * Math.max(1, power); i++)
                spawnRandomlyInsideAABB(()->random.nextDouble() < 0.75 ? particleMain : particleAlt, aabb,
                        1, 0,0.1f,0, 0.001f);
            ...
            if (hasPassedSectionsOf(maxAttacks)) {
                ...
                target.level().playSeededSound(null, target.getX(), target.getY(), target.getZ(),
                        SoundEvents.BEE_STING, SoundSource.HOSTILE,
